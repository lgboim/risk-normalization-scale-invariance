...
**Draft 7 — submission-oriented manuscript, September 1, 2026**  
...
![Figure 1. Annual gross payoff, fixed-dollar drag, and benchmark net payoff.](figures/figure1_annual_gross_drag_net.svg)
...
![Figure 2. Contract-scale downscaling frontier.](figures/figure2_downscaling_frontier.svg)
...
![Figure 3. Eligibility distributions and participation-quality decomposition.](figures/figure3_eligibility_decomposition.svg)
...
The primary source data are commercially licensed through the Sierra Chart Historical Data Service and cannot be redistributed. The operational extension uses separately licensed Databento data. The replication package will contain versioned analysis code, sanitized source manifests, derived audit tables, frozen protocols, the repair audit, and a hash-linked replication manifest. Reproducing the raw-data pipeline requires independent access to the licensed sources.
...
## Draft-completion register

The following items remain unresolved and must be completed from the licensed source inventory or for the selected journal rather than inferred from prose:

1. Count exact within-file duplicate minute timestamps in each raw contract file when the licensed source inventory is next available; do not treat expected timestamp overlap across different quarterly contracts as duplication and do not infer the within-file count from the zero-duplicate continuous artifacts.
2. Replace internal data-availability wording with the final anonymized repository link.
3. Apply target-journal reference style, declarations, word limits, and anonymization rules.
